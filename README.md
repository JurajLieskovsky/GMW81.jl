# GillMurrayWrightFactorization.jl
...
```
F = GillMurrayWrightFactorization.factorize(A)
```
or
```
F = GillMurrayWrightFactorization.factorize(A, δ) 
```
...
```
Ã = GillMurrayWrightFactorization.reconstruct(p, L)
```
or the in-place version
```
GillMurrayWrightFactorization.reconstruct!(Ã, p, L)
```
